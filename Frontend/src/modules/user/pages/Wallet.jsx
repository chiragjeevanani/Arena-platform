    ...
    <div className={`min-h-screen pb-32 ${isDark ? '' : 'bg-slate-50/50'}`}>
      ...
      <div className={`px-6 pt-5 pb-5 sticky top-0 z-50 backdrop-blur-xl border-b transition-all ${isDark ? 'bg-[#F3655D]/80 border-white/5' : 'bg-white border-blue-50 shadow-sm'}`}>
        <div className="flex items-center gap-4">
          ...
          <h1 className={`text-lg font-bold font-display ${isDark ? 'text-white' : 'text-[#F3655D]'}`}>My Wallet</h1>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start">
          ...
          <motion.div 
            ...
            className={`w-full md:w-[340px] md:aspect-square relative overflow-hidden p-5 md:p-6 rounded-2xl md:rounded-[32px] shadow-2xl shrink-0 flex flex-col justify-center ${
              isDark 
                ? 'bg-gradient-to-br from-[#F3655D] to-[#F3655D] border border-white/5' 
                : 'bg-gradient-to-br from-[#F3655D] to-[#122b5a] shadow-[0_20px_50px_rgba(10,31,68,0.25)]'
            }`}
          >
            ...
            <div className="relative z-10 flex flex-col items-center">
              ...
              <div className="mt-6 md:mt-8 w-full">
                <button className="w-full bg-white hover:bg-slate-50 text-[#F3655D] font-black py-3 md:py-3.5 rounded-xl md:rounded-2xl text-[9px] md:text-[10px] uppercase tracking-widest active:scale-95 transition-all flex items-center justify-center gap-2 group">
                  <div className="w-5 h-5 md:w-6 md:h-6 rounded-lg bg-[#F3655D] text-white flex items-center justify-center transition-transform group-hover:rotate-90">
          ...
          <div className="flex-1 w-full space-y-6">
            <div className="flex justify-between items-center px-1">
              <h3 className={`text-[11px] font-black uppercase tracking-[0.2em] ${isDark ? 'text-white/40' : 'text-[#F3655D]/40'}`}>Recent Transactions</h3>
              ...
              {transactions.map((tx, idx) => (
                ...
                >
                  ...
                  <div className="flex-1 text-left min-w-0">
                    <h4 className={`font-bold text-sm md:text-base tracking-tight truncate ${isDark ? 'text-white/80' : 'text-[#F3655D]'}`}>{tx.title}</h4>
                    <p className={`text-[10px] md:text-[11px] font-bold mt-0.5 opacity-50 flex items-center gap-1 ${isDark ? 'text-white' : 'text-[#F3655D]'}`}>
                      <Clock size={10} /> {tx.date}
                    </p>
                  </div>
                  <div className="text-right ml-3 md:ml-4">
                    <p className={`font-black text-sm md:text-base tracking-tight ${tx.type === 'received' ? 'text-emerald-500' : (isDark ? 'text-white/70' : 'text-[#F3655D]/70')}`}>
