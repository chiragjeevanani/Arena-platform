    ...
    <div className={`min-h-screen pb-32 ${isDark ? '' : 'bg-slate-50/50'}`}>
      ...
      <div className={`px-6 pt-5 pb-5 sticky top-0 z-50 backdrop-blur-xl border-b transition-all ${isDark ? 'bg-[#08142B]/80 border-white/5' : 'bg-white border-blue-50 shadow-sm'}`}>
        <div className="flex items-center gap-4">
          ...
          <h1 className={`text-lg font-bold font-display ${isDark ? 'text-white' : 'text-[#0A1F44]'}`}>Privacy & Security</h1>
        </div>
      </div>

      <div className="px-6 mt-8 space-y-6">
        <div className={`p-8 rounded-[40px] flex flex-col items-center text-center border ${isDark ? 'bg-[#08142B] border-white/5' : 'bg-[#0A1F44] text-white'}`}>
          <div className={`w-16 h-16 rounded-[22px] flex items-center justify-center mb-4 ${isDark ? 'bg-white/5 text-blue-400' : 'bg-white/10 text-white'}`}>
            <Shield size={32} />
          </div>
        ...
        <div className="space-y-4">
          {sections.map((section, idx) => (
            <motion.div
              ...
              className={`p-5 rounded-[28px] border transition-all ${isDark ? 'bg-white/5 border-white/5' : 'bg-white border-blue-50 shadow-sm'}`}
            >
        ...
        <div className="space-y-3 pt-4">
          <button className={`w-full p-4 rounded-2xl flex items-center justify-between border transition-all ${isDark ? 'bg-white/5 border-white/5' : 'bg-white border-blue-50 shadow-sm'}`}>
