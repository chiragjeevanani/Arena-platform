...
const ArenaDetails = () => {
  ...
  if (!arena) return <div className={`p-10 text-center ${isDark ? 'text-white/40' : 'text-[#0A1F44]/40'}`}>Arena not found</div>;
  ...
  return (
    <div className={`min-h-screen pb-32 relative overflow-hidden ${isDark ? '' : 'bg-white'}`}>
      ...
      <div className={`absolute top-[800px] -left-24 w-64 h-64 rounded-full blur-[100px] pointer-events-none ${isDark ? 'bg-[#22FF88]/10' : 'bg-[#22FF88]/[0.03]'}`} />

      <div className="relative h-[380px] overflow-hidden">
        <img src={arena.image} alt={arena.name} className="w-full h-full object-cover" />
        <div className={`absolute inset-0 bg-gradient-to-t ${isDark ? 'from-[#08142B] via-[#08142B]/30' : 'from-black/60 via-transparent'} to-transparent`} />
        <div className={`absolute inset-0 bg-gradient-to-b ${isDark ? 'from-[#08142B]/50' : 'from-black/40'} via-transparent to-transparent`} />
        ...
        <div className={`absolute bottom-6 left-6 right-6 backdrop-blur-xl rounded-3xl p-5 border shadow-2xl ${isDark ? 'bg-white/10 border-white/10' : 'bg-white border-white/20'}`}>
          <div className="flex justify-between items-start">
            <div>
              <h1 className={`text-xl font-bold font-display ${isDark ? 'text-white' : 'text-[#0A1F44]'}`}>{arena.name}</h1>
              <div className={`flex items-center text-xs mt-1 gap-1 ${isDark ? 'text-white/40' : 'text-[#0A1F44]/50'}`}>
            ...
            <div className={`px-3 py-1.5 rounded-xl flex items-center gap-1.5 ${isDark ? 'bg-white/10' : 'bg-[#FFD600]/10 border border-[#FFD600]/20'}`}>
              ...
              <span className={`text-sm font-black ${isDark ? 'text-white' : 'text-[#0A1F44]'}`}>{arena.rating}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 py-6 space-y-7">
        ...
        {/* Amenities */}
        <div>
          <h3 className={`text-base font-bold mb-4 font-display ${isDark ? 'text-white/80' : 'text-[#0A1F44]'}`}>Amenities</h3>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className={`text-base font-bold font-display ${isDark ? 'text-white/80' : 'text-[#0A1F44]'}`}>Select Court</h3>
