          ...
          return (
            <motion.button
              ...
              className={`
                ...
                ${isSelected
                  ? `bg-[#eb483f]/10 ${isDark ? 'border-[#eb483f]/40 neon-glow' : 'border-[#eb483f]/50 shadow-[0_0_10px_rgba(235, 72, 63,0.2)]'}`
            ...
            >
              ...
              <div className="relative w-full aspect-[2/3] mb-2">
                <div className={`absolute inset-0 border rounded-lg transition-colors duration-300 ${
                  isSelected ? 'border-[#eb483f]/30' : `${isDark ? 'border-white/10' : 'border-slate-200'}`
                }`} />
                <div className={`absolute left-0 right-0 top-1/2 h-[1px] transition-colors duration-300 ${
                  isSelected ? 'bg-[#eb483f]/30' : `${isDark ? 'bg-white/10' : 'bg-slate-100'}`
                }`} />
                <div className={`absolute top-0 bottom-0 left-1/2 w-[1px] transition-colors duration-300 ${
                  isSelected ? 'bg-[#eb483f]/20' : `${isDark ? 'bg-white/5' : 'bg-slate-50'}`
                }`} />
                ...
                {isSelected && (
                  ...
                  >
                    <div className="w-3 h-3 rounded-full bg-[#eb483f] animate-pulse-neon shadow-[0_0_10px_#eb483f]" />
                  </motion.div>
                )}
              </div>

              <span className={`text-[10px] font-bold block text-center transition-colors ${
                isSelected ? 'text-[#eb483f]' : `${isDark ? 'text-white/40' : 'text-[#F3655D]/60'}`
              }`}>
                {court.name.split(' ')[1]}
              </span>
              <span className={`text-[8px] font-bold block text-center mt-0.5 ${
                isSelected ? 'text-[#eb483f]/60' : `${isDark ? 'text-white/20' : 'text-slate-400'}`
              }`}>
                {court.type}
              </span>
            </motion.button>
