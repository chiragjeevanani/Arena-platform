  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className={`flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-3 border-b ${isDark ? 'border-white/5' : 'border-[#0A1F44]/10'}`}>
        <div>
          <h2 className={`text-xl md:text-2xl font-black font-display tracking-tight flex items-center gap-2 md:gap-3 ${isDark ? 'text-white' : 'text-[#0A1F44]'}`}>
            <PieChart className="text-[#eb483f] w-[20px] h-[20px] md:w-[24px] md:h-[24px]" /> Capital
          </h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4">
        {[
          { label: 'Gross', value: '₹1.24 Cr', change: '+24.5%', icon: DollarSign, color: '#eb483f' },
          { label: 'OPEX', value: '₹22.40 L', change: '-2.1%', icon: Wallet, color: '#FF4B4B' },
          { label: 'Net', value: '₹1.02 Cr', change: '+28.2%', icon: BarChart3, color: '#eb483f' },
          { label: 'Asset', value: '₹8.40 Cr', change: '+5.0%', icon: TrendingUp, color: '#eb483f' },
        ].map((stat, idx) => (
          <div key={idx} className={`p-2.5 md:p-5 rounded-xl md:rounded-3xl border ${isDark ? 'bg-[#0A1F44]/50 border-white/5' : 'bg-white border-[#0A1F44]/10 shadow-sm'}`}>
            ...
            <div className={`flex items-center gap-1 mt-1 text-[7px] md:text-[10px] font-black ${stat.change.startsWith('+') ? 'text-[#eb483f]' : 'text-[#FF4B4B]'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
              ...
              <AreaChart data={data}>
                <defs>
                  <linearGradient id="colorRev" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#eb483f" stopOpacity={0.1}/>
                ...
                />
                <Area type="monotone" dataKey="revenue" stroke="#eb483f" strokeWidth={2} fillOpacity={1} fill="url(#colorRev)" />
                <Area type="monotone" dataKey="coaching" stroke="#eb483f" strokeWidth={2} fillOpacity={0} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className={`p-4 md:p-8 rounded-xl md:rounded-[2.5rem] border ${isDark ? 'bg-[#0A1F44]/50 border-white/5' : 'bg-white border-[#0A1F44]/10 shadow-sm'}`}>
           ...
           <div className="space-y-5 md:space-y-6 text-[10px]">
              {[
                { label: 'GST Coll', value: '₹14.40L', color: '#eb483f' },
                { label: 'ITC Avail', value: '₹4.12L', color: '#eb483f' },
                { label: 'TDS Res', value: '₹56.1K', color: '#eb483f' },
              ].map((item, idx) => (
           ...
           <div className={`mt-6 md:mt-10 p-2.5 md:p-4 rounded-lg md:rounded-2xl border flex items-center justify-between group cursor-pointer transition-all ${isDark ? 'bg-white/2 border-white/5 hover:bg-white/5' : 'bg-black/2 border-black/5 hover:bg-black/5'}`}>
              <div className="flex items-center gap-2 md:gap-3">
                 ...
                 <div>
                    ...
                    <p className="text-[7px] md:text-[9px] font-black text-[#eb483f] uppercase italic">Verified</p>
                 </div>
              </div>
      ...
      <div className={`p-4 md:p-8 rounded-xl md:rounded-[2.5rem] border ${isDark ? 'bg-[#0A1F44]/50 border-white/5' : 'bg-white border-[#0A1F44]/10 shadow-sm'}`}>
         <div className="flex items-center justify-between mb-3 md:mb-6">
            ...
            <button className="text-[7px] md:text-[10px] font-black text-[#eb483f] uppercase tracking-widest italic hover:underline">Full Audit</button>
         </div>
            ...
            {[1, 2].map((i) => (
              <div key={i} className={`flex items-center justify-between p-2.5 md:p-4 rounded-lg md:rounded-2xl border transition-colors ${isDark ? 'bg-white/2 border-white/5 hover:bg-white/5' : 'bg-[#0A1F44]/2 border-[#0A1F44]/5 hover:bg-[#0A1F44]/5'}`}>
                 <div className="flex items-center gap-3 md:gap-4">
                    <div className={`w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl flex items-center justify-center border text-[#eb483f] ${isDark ? 'bg-[#eb483f]/10 border-[#eb483f]/10' : 'bg-[#eb483f]/10 border-[#eb483f]/20'}`}>
                 ...
                 <div className="text-right">
                    <p className={`text-[10px] md:text-sm font-black font-display ${i % 2 === 0 ? 'text-[#FF4B4B]' : 'text-[#eb483f]'}`}>{i % 2 === 0 ? '-' : '+'}₹8.2K</p>
              ...
              <div className="p-6 md:p-8 border-b border-inherit flex items-center justify-between">
                <div>
                  <h3 className="text-xl md:text-2xl font-black font-display tracking-tight flex items-center gap-2 md:gap-3">
                    <FileText className="text-[#eb483f] w-[20px] h-[20px] md:w-[24px] md:h-[24px]" /> Statement
                  </h3>
              ...
              <div className="p-6 md:p-8 space-y-4 md:space-y-5">
                <div className="grid grid-cols-2 gap-3 md:gap-4">
                  <div>
                    ...
                    <input type="date" className={`w-full py-3 md:py-4 px-3 md:px-4 rounded-xl md:rounded-2xl border text-[11px] md:text-xs font-bold outline-none transition-all ${isDark ? 'bg-white/5 border-white/5 focus:border-[#eb483f]/50 text-white' : 'bg-black/5 border-black/5 focus:border-[#eb483f] text-[#0A1F44]'}`} />
                  </div>
                  <div>
                ...
                <div className="group">
                  ...
                  <div className="relative">
                    <Mail size={12} className="absolute left-4 top-1/2 -translate-y-1/2 opacity-20 group-focus-within:text-[#eb483f] group-focus-within:opacity-100 transition-all" />
                    <input type="email" placeholder="finance@arena.com" className={`w-full py-3 md:py-4 pl-10 md:pl-12 pr-4 rounded-xl md:rounded-2xl border text-[11px] md:text-xs font-bold outline-none transition-all ${isDark ? 'bg-white/5 border-white/5 focus:border-[#eb483f]/50 text-white' : 'bg-black/5 border-black/5 focus:border-[#eb483f] text-[#0A1F44]'}`} />
                  </div>
                </div>
                <button
                  onClick={() => setShowExportModal(false)}
