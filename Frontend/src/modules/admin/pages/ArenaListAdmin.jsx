      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-slate-100">
        <div className="flex items-center gap-4">
           <button 
            ...
            className="w-10 h-10 rounded-xl border border-slate-100 flex items-center justify-center text-slate-400 hover:text-[#eb483f] hover:bg-[#eb483f]/5 transition-all"
          >
            <ArrowLeft size={18} />
          </button>
          <div>
        ...
        <button 
          ...
          className="bg-[#eb483f] text-white px-5 py-2 rounded-lg font-semibold text-[10px] uppercase tracking-widest shadow-lg hover:brightness-110 transition-all flex items-center gap-2"
        >
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3">
        <div className="flex-1 relative group w-full">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl text-xs border border-slate-100 bg-white outline-none focus:border-[#eb483f] transition-all font-semibold shadow-sm" 
          />
        </div>
        <button className="p-2.5 rounded-xl border border-slate-100 bg-white text-slate-400 hover:text-[#eb483f] transition-all shadow-sm">
          <Filter size={16} />
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6 pt-4">
        {filtered.map((arena) => (
          <motion.div
            ...
            className="group bg-white rounded-sm border border-slate-100 p-2.5 shadow-sm hover:shadow-lg hover:border-[#eb483f]/30 transition-all cursor-pointer relative overflow-hidden flex flex-col h-fit"
          >
            {/* Image Header */}
            <div className="relative h-28 rounded-sm overflow-hidden mb-2.5 bg-slate-50 border border-slate-50">
              ...
              <div className="absolute top-1.5 right-1.5 bg-[#eb483f] px-1.5 py-0.5 rounded-sm flex items-center gap-1">
              ...
              <div className="absolute bottom-1.5 left-1.5 flex gap-1 font-bold">
                 <span className="bg-white px-1.5 py-0.5 rounded-sm text-[8px] uppercase tracking-wider text-[#eb483f] font-black shadow-sm">
                   {arena.category}
                 </span>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <div>
                <h3 className="text-[13px] font-black text-slate-900 group-hover:text-[#eb483f] transition-all truncate tracking-tight uppercase">
                  {arena.name}
                </h3>
                <div className="flex items-center gap-1 mt-0.5 text-slate-400">
                  <MapPin size={9} className="text-[#eb483f]/60" />
              ...
              <div className="grid grid-cols-2 gap-1.5 py-2 border-y border-slate-50 px-1">
                <div className="flex items-center gap-2">
                   <div className="w-5 h-5 rounded-sm bg-[#eb483f]/5 flex items-center justify-center text-[#eb483f]/70">
                      <Target size={10} />
                   </div>
                   <div>
              ...
              <div className="flex items-center justify-between pt-0.5">
                <div>
                   <span className="text-[#eb483f] text-[11px] font-black tracking-tight">OMR {Number(arena.pricePerHour).toFixed(3)}</span>
                   <span className="text-[7px] text-slate-300 font-black uppercase ml-1 tracking-widest">/hr</span>
                </div>
                <button className="w-6 h-6 rounded-sm bg-slate-50 text-slate-200 group-hover:bg-[#eb483f] group-hover:text-white transition-all flex items-center justify-center border border-slate-100">
        ...
        <div 
          onClick={() => navigate('/admin/arena/details/new')}
          className="rounded-sm border-2 border-dashed border-slate-100 flex flex-col items-center justify-center py-8 gap-2.5 text-slate-300 hover:border-[#eb483f]/40 bg-slate-50/50 hover:bg-[#eb483f]/[0.02] group transition-all cursor-pointer h-[230px]"
        >
          <div className="w-9 h-9 rounded-sm bg-white flex items-center justify-center shadow-sm group-hover:scale-105 shadow-slate-100 transition-all border border-slate-100">
            <Plus size={18} className="text-[#eb483f]" />
          </div>
