      ...
      <div className="mx-auto space-y-4 py-4">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-3 pb-3 border-b border-slate-200 bg-white p-3 shadow-md rounded-sm">
           <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-sm bg-[#0A1121] flex items-center justify-center text-white shadow-sm">
                 <Target size={20} strokeWidth={2.5} />
              </div>
              <div>
                 <h2 className="text-lg font-black text-[#0A1121] uppercase leading-none">Court Management</h2>
           ...
           <div className="flex items-center gap-3">
              ...
              <button 
                 onClick={openAdd}
                 className="flex items-center justify-center gap-2 px-6 py-2.5 rounded-sm bg-[#eb483f] text-white hover:bg-[#d43d35] transition-all text-[9px] font-black uppercase tracking-widest shadow-lg shadow-[#eb483f]/20 active:translate-y-0.5"
              >
            ...
            <AnimatePresence>
              {courts.map((court, idx) => (
                <motion.div key={court.id}
                  ...
                  {/* Visual Accent */}
                  <div className="absolute top-0 right-0 w-16 h-16 bg-[#eb483f]/5 blur-2xl opacity-0 group-hover:opacity-100 transition-opacity" />
                  ...
                  <div className="flex items-center justify-between mb-4 relative z-10">
                     <div className="flex items-center gap-1.5 px-2 py-0.5 bg-slate-50 border border-slate-100 rounded-sm">
                        <span className="text-[7.5px] font-black text-[#0A1121] uppercase tracking-[0.2em]">NODE-0{idx + 1}</span>
                     </div>
                  ...
                  <div className="py-4 flex flex-col items-center justify-center space-y-4 relative z-10">
                     <div className="w-16 h-16 rounded-[4px] bg-[#F8FAFC] border border-slate-100 flex items-center justify-center text-[#94A3B8] group-hover:bg-[#0A1121] group-hover:border-[#0A1121] group-hover:text-white transition-all duration-300 shadow-sm relative">
                        <Grid3X3 size={32} strokeWidth={1.5} className="relative z-10" />
                        <div className="absolute inset-0 bg-gradient-to-tr from-[#eb483f]/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity rounded-[4px]" />
                     </div>
                     <div>
                        <h3 className="text-[13px] font-bold text-[#0A1121] uppercase tracking-wide text-center">{court.name}</h3>
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between relative z-10">
                     ...
                     <div className="flex gap-1">
                        <button onClick={() => openEdit(court)} className="w-8 h-8 flex items-center justify-center rounded-sm bg-slate-50 text-slate-500 hover:text-[#0A1121] hover:bg-slate-100 border border-transparent hover:border-slate-200 transition-all">
                           <Edit2 size={12} />
                        </button>
            ...
            <motion.div initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.98, opacity: 0 }}
              ...
              <div className="p-6 space-y-5 bg-[#F9FAFB]/30">
                <div className="space-y-1.5">
                  <label className="text-[8.5px] font-black uppercase tracking-widest text-[#0A1121] block">Designation (Title)</label>
                  <input type="text" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
                    placeholder="e.g. Court 5" className="w-full h-10 px-3 rounded-sm border border-slate-200 bg-white text-[11px] font-bold outline-none focus:border-[#eb483f] uppercase tracking-widest transition-all" />
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[8.5px] font-black uppercase tracking-widest text-[#0A1121] block">Locus Node Typology</label>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[8.5px] font-black uppercase tracking-widest text-[#0A1121] block">Status Registry</label>
                ...
                <div className="pt-2">
                  <button onClick={save} disabled={!form.name.trim()}
                    className="w-full h-12 rounded-sm bg-[#0A1121] text-white text-[10px] font-bold uppercase tracking-[0.25em] flex items-center justify-center gap-2 hover:bg-black transition-all disabled:opacity-30 disabled:cursor-not-allowed shadow-md">
      ...
      <AnimatePresence>
        {deleteId && (
          <div className="fixed inset-0 z-[130] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              onClick={() => setDeleteId(null)} className="absolute inset-0 bg-[#0A1121]/80 backdrop-blur-sm" />
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <h3 className="text-xl font-black text-[#0A1121] uppercase tracking-tight mb-2">Purge Resource?</h3>
