...
const PolicySettings = () => {
  ...
  const inputCls = "w-full py-3.5 px-4 rounded-xl border border-slate-200 bg-slate-50 text-[13px] font-bold outline-none focus:border-[#CE2029] focus:bg-white transition-all text-[#36454F]";
    ...
    <div className="grid grid-cols-1 xl:grid-cols-5 gap-8">
      ...
      <div className="xl:col-span-3 space-y-6">
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
           <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                 <h3 className="font-black text-[#36454F] text-lg uppercase tracking-widest">Refund Thresholds</h3>
              ...
              {refundRules.map((rule, idx) => (
                 <div key={idx} className="flex flex-col md:flex-row items-center gap-4 p-5 rounded-2xl border border-slate-100 bg-slate-50/50 hover:border-slate-200 transition-all">
                    ...
                    <div className="flex-1 space-y-2 w-full">
                       ...
                       <div className="relative">
                          <DollarSign size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#CE2029]" />
                    ...
                    <button className="p-3 text-slate-300 hover:text-[#CE2029] transition-all">
                       {rule.active ? <ToggleRight size={32} className="text-[#CE2029]" /> : <ToggleLeft size={32} />}
                    </button>
                 </div>
              ))}
           </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 space-y-6">
           <h3 className="text-xs font-black uppercase tracking-[0.2em] text-[#36454F] mb-6">General Arena Rules</h3>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="p-5 rounded-2xl border border-slate-100">
                 <div className="flex items-center gap-2 mb-4 text-[#CE2029]">
                    <Target size={16} />
        ...
        <motion.button
          ...
          className={`w-full py-5 rounded-3xl text-[12px] font-black uppercase tracking-[0.25em] shadow-lg flex items-center justify-center gap-3 transition-all ${
            saved ? 'bg-green-500 text-white shadow-green-500/20' : 'bg-[#36454F] text-white hover:shadow-[#36454F]/30 hover:-translate-y-0.5'
          }`}
        >
          {saved ? (
      ...
      <div className="xl:col-span-2">
         <div className="bg-slate-900 p-8 rounded-3xl shadow-xl border border-white/5 sticky top-4">
            <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-[#CE2029] mb-8">Rule Documentation</h3>
            ...
            <div className="space-y-8">
               {[
                 ...
                 <div key={i} className="flex gap-4 group">
                    <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white/50 group-hover:text-[#CE2029] group-hover:border-[#CE2029]/30 transition-all shrink-0">
                       <doc.icon size={18} />
                    </div>
                    <div>
                       <p className="text-sm font-black text-white group-hover:text-[#CE2029] transition-all uppercase tracking-tight mb-1">{doc.title}</p>
            ...
            <div className="mt-12 p-6 bg-[#CE2029]/10 border border-[#CE2029]/20 rounded-2xl">
               <div className="flex items-center gap-2 mb-2 text-[#CE2029]">
                  <Shield size={16} />
                  <span className="text-[10px] font-black uppercase tracking-widest">Legal Notice</span>
               </div>
               <p className="text-[10px] font-bold text-[#CE2029]/70 leading-relaxed uppercase">Updates to refund policies are sent via email to all active users immediately upon saving.</p>
            </div>
         </div>
      </div>
    </div>
