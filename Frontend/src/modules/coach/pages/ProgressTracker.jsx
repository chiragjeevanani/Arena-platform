...
import {
  ...
  Award, Target, Activity, BookOpen, BarChart2, X, CheckCircle2
} from 'lucide-react';
...
const ProgressTracker = () => {
  ...
  const filtered = [...STUDENTS_PROGRESS]
    .filter(s =>
  ...
  const avg = (arr, key) => Math.round(arr.reduce((a, s) => a + s[key], 0) / arr.length);
  ...
  return (
    <div className="space-y-3 max-w-[1600px] mx-auto relative px-1">
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-[#1a2b3c] border border-slate-700 text-white px-4 py-2 rounded-xl shadow-2xl flex items-center gap-2 min-w-[240px]"
          >
      ...
      <div className={`flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 pb-3 border-b ${isDark ? 'border-white/5' : 'border-slate-200'}`}>
        <div>
          <h2 className={`text-xl font-semibold tracking-tight flex items-center gap-2 ${isDark ? 'text-white' : 'text-[#1a2b3c]'}`}>
            <TrendingUp className="text-[#eb483f]" size={20} /> Progress Tracker
          </h2>
          <p className={`text-[10px] mt-0.5 font-medium ${isDark ? 'text-white/40' : 'text-slate-500'}`}>
            Per-student performance metrics, skill ratings, and attendance history.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {[
          { label: 'Students', value: STUDENTS_PROGRESS.length, icon: User, color: '#eb483f' },
          { label: 'Avg Overall', value: `${avg(STUDENTS_PROGRESS, 'overall')}%`, icon: Target, color: '#6366f1' },
          { label: 'Avg Attendance', value: `${avg(STUDENTS_PROGRESS, 'attendance')}%`, icon: Activity, color: '#22c55e' },
          { label: 'Total Remarks', value: STUDENTS_PROGRESS.reduce((a, s) => a + s.remarks, 0), icon: BookOpen, color: '#f59e0b' },
        ].map((s, i) => (
          <div key={i} className={`p-2.5 rounded-xl border shadow-sm flex items-center gap-2.5 ${isDark ? 'bg-[#1a1d24] border-white/5' : 'bg-white border-slate-100'}`}>
            ...
            <div>
              ...
              <p className={`text-lg font-bold tracking-tight leading-tight ${isDark ? 'text-white' : 'text-[#1a2b3c]'}`}>{s.value}</p>
            </div>
          </div>
        ))}
      </div>
          ...
          {['overall', 'attendance', 'name'].map(opt => (
            <button key={opt} onClick={() => setSortBy(opt)}
              className={`px-3 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-widest border transition-all ${
                sortBy === opt
                  ? 'bg-[#eb483f] text-white border-[#eb483f] shadow-sm'
                  : isDark ? 'bg-white/5 border-white/10 text-white/40 hover:bg-white/10' : 'bg-white border-slate-200 text-slate-500 hover:border-[#eb483f]'
              }`}
            >{opt}</button>
          ))}
        </div>
            ...
            >
              ...
              >
                ...
                <div className="w-8 h-8 rounded-xl flex items-center justify-center font-bold text-xs shrink-0 text-white"
                  style={{ backgroundColor: LEVEL_COLOR[student.level] }}>
                  {student.avatar}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 leading-none">
                    <h4 className={`font-bold text-sm tracking-tight truncate ${isDark ? 'text-white' : 'text-[#1a2b3c]'}`}>{student.name}</h4>
                ...
                <div className="flex items-center gap-4 sm:gap-6 shrink-0">
                  <div className="text-center hidden sm:block">
                    ...
                    <p className={`text-base font-bold leading-none ${student.overall >= 85 ? 'text-[#eb483f]' : isDark ? 'text-white' : 'text-[#1a2b3c]'}`}>{student.overall}%</p>
                  </div>
                  <div className="text-center hidden sm:block">
                  ...
                  <div className={`transition-transform duration-300 ${isOpen ? 'rotate-180 text-[#eb483f]' : 'text-slate-400'}`}>
                    <ChevronDown size={16} />
                  </div>
                </div>
              </button>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-2">
                        ...
                        <div>
                          <p className={`text-[9px] font-semibold uppercase tracking-widest mb-3 flex items-center gap-1.5 ${isDark ? 'text-white/40' : 'text-slate-400'}`}>
                            <BarChart2 size={11} className="text-[#eb483f]" /> Skill Breakdown
                          </p>
                          <div className="space-y-1">
                            {student.skills.map(s => (
                              <SkillBar key={s.name} name={s.name} score={s.score} isDark={isDark} />
                            ))}
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-3">
                          <div className="grid grid-cols-2 gap-2">
                            <div className={`p-2.5 rounded-xl border ${isDark ? 'bg-white/[0.02] border-white/10' : 'bg-white border-slate-200'}`}>
                              <p className={`text-[8px] font-semibold uppercase tracking-widest mb-1 text-[#eb483f]`}>Attendance</p>
                              <div className="flex items-baseline gap-1">
                                <span className={`text-xl font-bold ${student.attendance >= 90 ? 'text-green-500' : 'text-amber-500'}`}>{student.attendance}%</span>
                                <span className={`text-[8px] font-semibold ${isDark ? 'text-white/20' : 'text-slate-400'}`}>Total</span>
                              </div>
                            </div>
                            <div className={`p-2.5 rounded-xl border ${isDark ? 'bg-white/[0.02] border-white/10' : 'bg-white border-slate-200'}`}>
                              <p className={`text-[8px] font-semibold uppercase tracking-widest mb-1 text-[#eb483f]`}>Monthly Trend</p>
                              <div className={`text-xl font-bold flex items-center gap-1 ${trendUp ? 'text-green-500' : 'text-red-400'}`}>
                                {trendUp ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                                {student.trend}
                              </div>
                            </div>
                          </div>
                          
                          <div className={`flex-1 p-3 rounded-xl border flex flex-col justify-center ${isDark ? 'bg-white/[0.02] border-white/10' : 'bg-white border-slate-200'}`}>
                            <p className={`text-[8px] font-semibold uppercase tracking-widest mb-1 ${isDark ? 'text-white/20' : 'text-slate-400'}`}>Top Achievement</p>
                            <p className={`text-xs font-bold italic ${isDark ? 'text-white/80' : 'text-[#1a2b3c]'}`}>
                              "Exhibits exceptional {student.skills.sort((a,b) => b.score - a.score)[0].name.toLowerCase()} in all drills"
                            </p>
                          </div>

                          <div className="flex gap-2">
                            <button
                              onClick={() => showToast(`Opening performance logs for ${student.name}...`)}
                              className={`flex-1 py-1.5 rounded-xl text-[9px] font-bold uppercase tracking-widest border transition-all ${isDark ? 'border-white/10 text-white/50 hover:bg-white/5' : 'border-slate-200 text-slate-500 hover:border-[#eb483f] hover:text-[#eb483f]'}`}
                            >
                              Details
                            </button>
                            <button
                              onClick={() => showToast(`Generating PDF report for ${student.name}...`)}
                              className="flex-1 py-1.5 rounded-xl bg-[#eb483f] text-white text-[9px] font-bold uppercase tracking-widest hover:bg-[#1a2b3c] transition-all shadow-md shadow-[#eb483f]/20"
                            >
                              Export
                            </button>
                          </div>
