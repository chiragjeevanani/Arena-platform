...
const SkillBar = ({ name, score, isDark }) => (
  <div className="mb-2">
    <div className="flex justify-between items-center mb-1">
      ...
      <span className={`text-[10px] font-bold ${score >= 85 ? 'text-[#eb483f]' : isDark ? 'text-white/70' : 'text-slate-700'}`}>{score}%</span>
    </div>
    <div className={`h-1.5 rounded-full ${isDark ? 'bg-white/10' : 'bg-slate-100'}`}>
      <motion.div
        ...
        style={{ backgroundColor: score >= 85 ? '#eb483f' : score >= 70 ? '#f59e0b' : '#94a3b8' }}
      />
    </div>
  </div>
);
    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
      ...
      <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }}
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h3 className={`text-lg font-black tracking-tight ${isDark ? 'text-white' : 'text-[#1a2b3c]'}`}>{student.name}</h3>
            <p className="text-[10px] font-black uppercase text-[#eb483f] tracking-widest">{student.batch}</p>
          </div>
        </div>
          ...
          {skills.map((skill) => (
            <div key={skill.name}>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className={`text-sm font-black ${isDark ? 'text-white' : 'text-[#1a2b3c]'}`}>{skill.score}%</span>
              </div>
              <input type="range" min="0" max="100" value={skill.score} onChange={(e) => updateScore(skill.name, e.target.value)}
                className="w-full accent-[#eb483f] h-1.5 bg-slate-100 rounded-full appearance-none cursor-pointer" />
            </div>
          ))}
        </div>
        <button onClick={handleSave} disabled={saving} className="w-full mt-10 py-4 rounded-2xl bg-[#eb483f] text-white text-[11px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-3 active:scale-95 transition-all shadow-xl shadow-[#eb483f]/20">
...
const ProgressTracker = () => {
  ...
  return (
    <div className="space-y-3 max-w-[1600px] mx-auto relative px-1">
      <AnimatePresence>{toast && <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[150] bg-[#1a2b3c] border border-slate-700 text-white px-4 py-2 rounded-xl shadow-2xl flex items-center gap-2 min-w-[240px]"><CheckCircle2 size={16} className="text-[#eb483f]" /><span className="text-[10px] font-bold uppercase tracking-widest">{toast}</span></motion.div>}</AnimatePresence>
      ...
      <div className={`flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 pb-3 border-b ${isDark ? 'border-white/5' : 'border-slate-200'}`}>
        <div><h2 className={`text-xl font-semibold tracking-tight flex items-center gap-2 ${isDark ? 'text-white' : 'text-[#1a2b3c]'}`}><TrendingUp className="text-[#eb483f]" size={20} /> Progress Tracker</h2><p className={`text-[10px] mt-0.5 font-medium ${isDark ? 'text-white/40' : 'text-slate-500'}`}>Per-student performance metrics, skill ratings, and attendance history.</p></div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {[
          { label: 'Students', value: students.length, icon: User, color: '#eb483f' },
          ...
          <div key={i} className={`p-2.5 rounded-xl border shadow-sm flex items-center gap-2.5 ${isDark ? 'bg-[#1a1d24] border-white/5' : 'bg-white border-slate-100'}`}>
            ...
            <div><p className={`text-[7px] font-semibold uppercase tracking-widest leading-none mb-0.5 ${isDark ? 'text-white/30' : 'text-slate-400'}`}>{s.label}</p><p className={`text-lg font-bold tracking-tight leading-tight ${isDark ? 'text-white' : 'text-[#1a2b3c]'}`}>{s.value}</p></div>
          </div>
        ))}
      </div>

      <div className="flex flex-col sm:flex-row items-center gap-2">
        <div className="flex-1 relative group w-full"><Search size={13} className={`absolute left-3 top-1/2 -translate-y-1/2 ${isDark ? 'text-white/20' : 'text-slate-400'}`} /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name or batch..." className={`w-full pl-9 pr-4 py-1.5 rounded-lg text-[11px] border outline-none transition-all ${isDark ? 'bg-white/5 border-white/5 text-white placeholder:text-white/20 focus:border-[#eb483f]' : 'bg-white border-slate-200 text-[#1a2b3c] focus:border-[#eb483f]'}`} /></div>
        <div className="flex items-center gap-1 shrink-0"><span className={`text-[8px] font-semibold uppercase tracking-widest mr-1 ${isDark ? 'text-white/30' : 'text-slate-400'}`}>Sort:</span>{['overall', 'attendance', 'name'].map(opt => (<button key={opt} onClick={() => setSortBy(opt)} className={`px-3 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-widest border transition-all ${sortBy === opt ? 'bg-[#eb483f] text-white border-[#eb483f] shadow-sm' : isDark ? 'bg-white/5 border-white/10 text-white/40 hover:bg-white/10' : 'bg-white border-slate-200 text-slate-500 hover:border-[#eb483f]'}`}>{opt}</button>))}</div>
      </div>
                  ...
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                    <div className={`px-4 pb-4 pt-2 border-t ${isDark ? 'border-white/5' : 'border-slate-100'} ${isDark ? 'bg-white/[0.01]' : 'bg-slate-50/30'}`}>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-2">
                        <div><p className={`text-[9px] font-semibold uppercase tracking-widest mb-3 flex items-center gap-1.5 ${isDark ? 'text-white/40' : 'text-slate-400'}`}><BarChart2 size={11} className="text-[#eb483f]" /> Skill Breakdown</p><div className="space-y-1">{student.skills.map(s => (<SkillBar key={s.name} name={s.name} score={s.score} isDark={isDark} />))}</div></div>
                        <div className="flex flex-col gap-3">
                          <div className="grid grid-cols-2 gap-2"><div className={`p-2.5 rounded-xl border ${isDark ? 'bg-white/[0.02] border-white/10' : 'bg-white border-slate-200'}`}><p className={`text-[8px] font-semibold uppercase tracking-widest mb-1 text-[#eb483f]`}>Attendance</p><div className="flex items-baseline gap-1"><span className={`text-xl font-bold ${student.attendance >= 90 ? 'text-green-500' : 'text-amber-500'}`}>{student.attendance}%</span><span className={`text-[8px] font-semibold ${isDark ? 'text-white/20' : 'text-slate-400'}`}>Total</span></div></div><div className={`p-2.5 rounded-xl border ${isDark ? 'bg-white/[0.02] border-white/10' : 'bg-white border-slate-200'}`}><p className={`text-[8px] font-semibold uppercase tracking-widest mb-1 text-[#eb483f]`}>Monthly Trend</p><div className={`text-xl font-bold flex items-center gap-1 ${trendUp ? 'text-green-500' : 'text-red-400'}`}>{trendUp ? <ChevronUp size={16} /> : <ChevronDown size={16} />}{student.trend}</div></div></div>
                          <div className={`flex-1 p-3 rounded-xl border flex flex-col justify-center ${isDark ? 'bg-white/[0.02] border-white/10' : 'bg-white border-slate-200'}`}><p className={`text-[8px] font-semibold uppercase tracking-widest mb-1 ${isDark ? 'text-white/20' : 'text-slate-400'}`}>Top Achievement</p><p className={`text-xs font-bold italic ${isDark ? 'text-white/80' : 'text-[#1a2b3c]'}`}>"Exhibits exceptional {student.skills.slice().sort((a,b) => b.score - a.score)[0].name.toLowerCase()} in all drills"</p></div>
                          <div className="flex gap-2">
                            <button onClick={() => setEditStudent(student)} className={`flex-1 py-1.5 rounded-xl text-[9px] font-bold uppercase tracking-widest border transition-all ${isDark ? 'bg-[#eb483f] text-white border-[#eb483f]' : 'bg-[#1a2b3c] text-white border-[#1a2b3c] hover:bg-[#eb483f]'}`}>Update Skills</button>
                            <button onClick={() => showToast(`Generating PDF report for ${student.name}...`)} className={`flex-1 py-1.5 rounded-xl text-[9px] font-bold uppercase tracking-widest border transition-all ${isDark ? 'border-white/10 text-white/50 hover:bg-white/5' : 'border-slate-200 text-slate-500 hover:border-[#eb483f]'}`}>Export PDF</button>
                          </div>
                        </div>
                      </div>
                    </div>
